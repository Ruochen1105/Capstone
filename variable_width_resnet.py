...
class VariableWidthResNet(nn.Module):
    def __init__(self, block, layers, input_channels=3, width=64, num_classes=10, zero_init_residual=False,
...
class AttentionedResNet(nn.Module):
    def __init__(self, block, layers, input_channels=3, width=64, num_classes=10, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, initial_kernel_size=7, initial_stride=2, initial_padding=3,
                 use_initial_max_pooling=True, width_factors=[1, 2, 4, 8]):
        super(AttentionedResNet, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
    ...
    def _forward_impl(self, x):
        ...
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        auxiliary_attention = self.auxiliary_mlp(x)
        attention = self.mlp(x)
